...
包含 底部弹出视图(Dialog)、支付密码输入框(PassWordInput)、商品数量加减(Quantity)、提示消息(Toast)、顶部提示(Toptip)、角标(Badge)、星级评分(Rater)、图表(Charts)、富文本解析(RichText)、图片截取(Cropper)共计 10 类组件或元素。
...
注：本项目基础库版本为1.5.4，如运行失败，请尝试升级更新【微信开发者工具】。
